### gtm-k — Gowtham Kethineedi

> Execution keeps getting cheaper. Judgment doesn't.
> So I productize the judgment — PM, strategy, finance — and run experiments at the frontier.
> Built in public, on my own time.

**Shipped — in market**
- 📱 **[gunafood](https://gtm-k.github.io/gunafood)** — Ayurvedic food-wisdom app: 433 foods classified as Sattvic, Rajasic, or Tamasic via the Guna framework, with citations and timing guidance. iOS & Android. *(closed source)*

**Built — open**
- 🧠 **[product-skill](https://github.com/gtm-k/product-skill)** ⭐ — 30+ PM frameworks as an AI skill. Product judgment, on tap. *(8.9/10 across blind scenario tests)*
- 📊 **[fintree](https://github.com/gtm-k/fintree)** — A 234-node, XBRL-mapped US GAAP P&L ontology for FP&A teams and AI agents.
- ⚛️ **[quantum-fusion-poc](https://github.com/gtm-k/quantum-fusion-poc)** — VQE on tungsten–hydrogen binding, validated on real IBM Quantum hardware. A frontier proof-of-concept — including what doesn't work yet.
- 🛠 **[skill-forge](https://github.com/gtm-k/skill-forge)** — single-file AI agent skill manager. Visual editor, quality scorer, cross-agent sync.

**In progress**
- 🗂 **[foldermcp](https://github.com/gtm-k/foldermcp)** — folders → secure MCP tool servers (Go). *Evolving — next version in the works.*
- 🏛 **[strategy-council](https://github.com/gtm-k/strategy-council)** — an experiment in multi-agent strategic reasoning. *Research, not production.*

**Now** — reworking the next version of `foldermcp`.

---

Product leader · 10+ yrs · [gowthamkethineedi.com](https://www.gowthamkethineedi.com) · [LinkedIn](https://www.linkedin.com/in/gowthamkethineedi)

<sub>Personal projects, built on my own time. Views my own.</sub>
